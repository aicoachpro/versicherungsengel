...
// VOE-191: Datenquelle ist jetzt seeds/check-direkt-sparten.ts (auch von
// runtime-migrations.ts genutzt). Auf Prod wird der Seed bereits beim
// Container-Start ausgefuehrt — diesen Block hier behalten wir nur fuer
// manuelle Re-Seeds via `tsx src/db/migrate.ts`.
...
if (existingCount < 50) {
  ...
  const insertStmt = sqlite.prepare(
    "INSERT INTO lead_products (name, kuerzel, sort_order) SELECT ?, ?, ? WHERE NOT EXISTS (SELECT 1 FROM lead_products WHERE kuerzel = ?)"
  );
  let inserted = 0;
  let sortIdx = 100;
